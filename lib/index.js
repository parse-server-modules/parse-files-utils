...
function initialize(config) {
  questions(config).then(function (answers) {
    ...
    objects = filterPreviousUploads(config.successLogFilePath, objects);
...
function filterPreviousUploads(uploadListPath, objects) {
  var uploadedFileList;
  if (uploadListPath) {
    uploadedFileList = fs.readFileSync(uploadListPath, {encoding: 'utf8'});
    uploadedFileList = uploadedFileList.split('\n');
    if (!uploadedFileList[uploadedFileList.length-1]) uploadedFileList.pop();

    uploadedFileList = _.map(uploadedFileList, function(anUploadedFile) {
      return JSON.parse(anUploadedFile);
    });

    uploadedFilesById = _.groupBy(uploadedFileList, 'objectId');
  }

  objects = _.filter(objects, function(aFileObject) {
    if (!aFileObject.fileName) return false; //null fileName causes error in indexOf later

    if (!uploadedFileList) return true; //no previously uploaded list so not filter
    if (!uploadedFilesById[aFileObject.objectId]) return true; //Id not in map, so not previously uploaded

    var match = _.find(uploadedFilesById[aFileObject.objectId], function(anUploadedFile) {
      return (anUploadedFile.url == aFileObject.url && anUploadedFile.objectId == aFileObject.objectId && anUploadedFile.fieldName == aFileObject.fieldName);
    });
    return !match;
  });
    ...
    return results.reduce(function(current, result){
      return current.concat(
        schema.fields.map(function(field){
          ...
          if (result.get(field) && result.get(field) != null) {
            returnValue.fileName = result.get(field).name();
            returnValue.url = result.get(field).url();
          }
          return returnValue;
        })
      );
